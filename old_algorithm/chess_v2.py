...
class AI(object):
    ...
    def go(self, chessboard):
        ...
        if self.use_ab:
            action = self.ab_search(chessboard, d=3, eval_fn=self.eval_fn)
            if action:
    ...
    def __random_p(self, chessboard):
        ...
        if chessboard[mid][mid] == COLOR_NONE:
            self.candidate_list.append((mid, mid))
            return
        random.seed(time.time())
        ...
        for i in range(self.chessboard_size):
            for j in range(self.chessboard_size):
                ...
                if score >= 5 or score2 >= 5:
                    des.append((i, j, max(score, score2), min(score, score2)))
                if score > self.max_self:
                    self.max_self = score
                if score2 > self.max_other:
                    self.max_other = score2
                # s = max(score, score2)
                # if s > 0:
                #     des.append((i, j, s))
        # des.sort(key=lambda x: x[2])
    ...
    def eval_fn(self, state, fifq, color):
        ...
        weight = np.array([1600, 10, 20, 50, 1600, 20, 50, 1000, 1600])
        # weight2 = weight - 10
        ...
        if self.max_self > 300:
            return score1 - score2 - 400
        else:
